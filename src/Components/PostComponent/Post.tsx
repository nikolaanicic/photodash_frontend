...
export const Post = (props: any) => {
  ...
  const [postElement, setPostElement] = useState<JSX.Element | undefined>();
  ...
  const handleLike = (postId: string, username: string) => {
    ...
    fetch(`/api/${username}/posts/like/${postId}`, reqOptions)
      .then((response) => {
      ...
      <div className="post-container" key={post.id}>
        ...
        <div className="row-container">
          <div>
            {post.isLiked
              ? Button("&#9829;", () => {
                  handleLike("A798B7F7-E78C-4F65-808C-84AB531B0EE0", "nikola");
                })
              : Button("&#x2715;", switchLikeStatus)}
          </div>
          <div>
            {Button("&#128488;", () =>
              handleGetComments("A798B7F7-E78C-4F65-808C-84AB531B0EE0")
            )}
          </div>
        </div>
      </div>
  ...
  useEffect(() => {
    setPostElement(renderPost(mockPost));
  }, []);

  return Card(postElement, false);
};
